...
const foto = path.join(ROOT, 'foto.webp');
...
const tmpHtml = path.join(os.tmpdir(), 'ig-arte-' + Date.now() + '.html');
fs.writeFileSync(tmpHtml, html, 'utf8');

const out = path.join(ROOT, 'exemplo-instagram.png');
...
if (!fs.existsSync(out)) {
  throw new Error('Falhou: PNG não foi gerado');
}
console.log('OK ->', out);
